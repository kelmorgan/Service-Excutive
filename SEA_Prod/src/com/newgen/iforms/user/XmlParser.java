/*---------------------------------------------------------------------------------------------------------------------------
	...
	Application                  : CI Process
	Module                       : XmlParser
	File Name                    : XmlParser.java
	Author                       : Anil Jee Ojha
	Date (DD/MM/YYYY)            : 19/09/2019
	Description                  : This file consist of common methods of XML Parsing for CI Process
	------------------------------------------------------------------------------------------------------------------------------
	CHANGE HISTORY
	------------------------------------------------------------------------------------------------------------------------------
...
public class XmlParser {
	...
	public String getValueOf(String paramString) {
		try {
			return new String(this.copyString.substring(
					this.parseString.indexOf("<"
							+ toUpperCase(paramString, 0, 0) + ">")
	...
	public String getValueOf(String paramString, int paramInt1, int paramInt2) {
		try {
			if (paramInt1 >= 0) {
				int i = this.parseString.indexOf(
						"</" + toUpperCase(paramString, 0, 0) + ">", paramInt1);
				if ((i > paramInt1) && ((paramInt2 == 0) || (paramInt2 >= i)))
					return new String(this.copyString.substring(
							this.parseString.indexOf(
									"<" + toUpperCase(paramString, 0, 0) + ">",
									paramInt1) + paramString.length() + 2, i));
			}
			return "";
		} catch (StringIndexOutOfBoundsException localStringIndexOutOfBoundsException) {
		}
	...
	public String getFirstValueOf(String paramString) {
		try {
			this.IndexOfPrevSrch = this.parseString.indexOf("<"
					+ toUpperCase(paramString, 0, 0) + ">");
			return new String(this.copyString.substring(
					this.IndexOfPrevSrch + paramString.length() + 2,
					this.parseString.indexOf("</"
							+ toUpperCase(paramString, 0, 0) + ">")));
		} catch (StringIndexOutOfBoundsException localStringIndexOutOfBoundsException) {
		}
	...
	public String getFirstValueOf(String paramString, int paramInt) {
		try {
			this.IndexOfPrevSrch = this.parseString.indexOf(
					"<" + toUpperCase(paramString, 0, 0) + ">", paramInt);
			return new String(this.copyString.substring(this.IndexOfPrevSrch
					+ paramString.length() + 2, this.parseString.indexOf("</"
					+ toUpperCase(paramString, 0, 0) + ">", paramInt)));
		} catch (StringIndexOutOfBoundsException localStringIndexOutOfBoundsException) {
		}
		return "";
	...
	public String convertToSQLString(String paramString) {
		try {
			for (int i = paramString.indexOf("["); i != -1; i = paramString
					.indexOf("[", i + 2))
				paramString = paramString.substring(0, i) + "[[]"
						+ paramString.substring(i + 1, paramString.length());
		} catch (Exception localException1) {
		}
		try {
			for (int j = paramString.indexOf("_"); j != -1; j = paramString
					.indexOf("_", j + 2))
				paramString = paramString.substring(0, j) + "[_]"
						+ paramString.substring(j + 1, paramString.length());
		} catch (Exception localException2) {
		}
		try {
			for (int k = paramString.indexOf("%"); k != -1; k = paramString
					.indexOf("%", k + 2))
				paramString = paramString.substring(0, k) + "[%]"
						+ paramString.substring(k + 1, paramString.length());
	...
	public String getValueOf(String paramString1, String paramString2,
		...
		try {
			if (paramString2.equalsIgnoreCase("Binary")) {
				...
				if (i == -1)
					return "";
				int j = this.copyString.indexOf("</" + paramString1 + ">",
						paramInt1);
				if (j > paramInt2)
					return "";
				i += new String("<" + paramString1 + ">").length();
	...
	public void changeValue(String paramString1, String paramString2) {
		try {
			...
			if (i > -1) {
				i += str1.length();
				j = this.parseString.indexOf(("</" + paramString1 + ">")
						.toUpperCase());
				str2 = this.copyString.substring(0, i);
				this.copyString = (str2 + paramString2 + this.copyString
						.substring(j));
			} else {
				j = i = this.parseString.lastIndexOf("</");
				str2 = this.copyString.substring(0, i);
				this.copyString = (str2 + "<" + paramString1 + ">"
						+ paramString2 + "</" + paramString1 + ">" + this.copyString
						.substring(j));
			}
			this.parseString = toUpperCase(this.copyString, 0, 0);
		} catch (Exception localException) {
	...
	public String ParseFieldValue(String paramString1, String paramString2,
			String paramString3) {
		try {
			int i = this.parseString.indexOf("<"
					+ toUpperCase(paramString1, 0, 0) + ">" + "<"
					+ toUpperCase(paramString2, 0, 0) + ">"
					+ toUpperCase(paramString3, 0, 0) + "</"
					+ toUpperCase(paramString2, 0, 0) + ">");
			if (i != -1) {
				i += paramString1.length() + 2;
				return new String(this.copyString.substring(i, this.parseString
						.indexOf("</" + toUpperCase(paramString1, 0, 0) + ">",
								i)));
			}
			return "";
		} catch (StringIndexOutOfBoundsException localStringIndexOutOfBoundsException) {
	...
	public String getValueListXml(String paramString) {
		try {
			int i = this.parseString.indexOf("<NAME>"
					+ paramString.toUpperCase() + "</NAME>");
			if (i != -1) {
	...
	public String getFieldValue(String paramString) {
		try {
			...
			if (i != -1) {
				...
				String str = this.copyString.substring(i + 7,
						this.parseString.indexOf("</VALUE>", i));
				return str;
			}
		} catch (StringIndexOutOfBoundsException localStringIndexOutOfBoundsException) {
			localStringIndexOutOfBoundsException.printStackTrace();
